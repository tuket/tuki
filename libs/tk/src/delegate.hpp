...
namespace tk
{
	...
	public:
		...
		{
            return (*invocation.stub)(invocation.object, std::forward<PARAMS_FN>(args)...);
		}
    ...
    #define make_delegate(obj, memberFn) tk::_make_delegate<decltype(memberFn), memberFn>(obj)
    #define make_plain_delegate(plainFn) tk::_make_delegate<decltype(plainFn), plainFn>()
}
